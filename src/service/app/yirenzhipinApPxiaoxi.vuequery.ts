...
/** 获取聊天会话列表 GET /app-api/yirenzhipin/message/get-chat-session-list */
export function getChatSessionListQueryOptions(options: {
  options?: CustomRequestOptions;
}) {
  return queryOptions({
    queryFn: async ({ queryKey }) => {
      return apis.getChatSessionList(queryKey[1] as typeof options);
    },
    queryKey: ['getChatSessionList', options],
  });
}

/** 获取系统消息列表 GET /app-api/yirenzhipin/message/get-system-session-list */
export function getSystemMessageListQueryOptions(options: {
  options?: CustomRequestOptions;
}) {
  return queryOptions({
    queryFn: async ({ queryKey }) => {
      return apis.getSystemMessageList(queryKey[1] as typeof options);
    },
    queryKey: ['getSystemMessageList', options],
  });
}
